...
- **Routing** (`App.tsx`): three areas — public (`/login`, `/registro`, `/catalogo/:slug`), the **admin panel** under `/admin` (per-store back office, gated by `<ProtectedRoute rol="ADMIN">`; pages in `pages/admin/`), and the **marketplace** under `/tienda` (`MarketplaceLayout` wraps home/search/store/info pages from `pages/marketplace/` plus the product-detail/cart/checkout/orders pages from `pages/tienda/`; public browsing, `CartProvider` scopes the cart, checkout/cart/orders gated by `<ProtectedRoute rol="CLIENTE">`). `/tienda` is the multi-store marketplace ("Klikea"), not a single shop — `comercio/:slug` is an individual store's page within it.
