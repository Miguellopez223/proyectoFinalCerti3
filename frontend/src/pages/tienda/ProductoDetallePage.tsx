        ...
        {producto && (
          <div className="grid gap-8 lg:grid-cols-2">
            ...
            <div>
              ...
              <h1 className="mt-3 text-2xl font-semibold text-slate-900">{producto.nombre}</h1>
              ...
              {producto.descripcionLarga && (
                <p className="mt-4 whitespace-pre-line text-sm leading-relaxed text-slate-600">{producto.descripcionLarga}</p>
              )}
                ...
                <div className="mt-6 flex items-center gap-3">
                  <div className="flex items-center rounded-lg border border-slate-300">
                    <button
                      ...
                      className="cursor-pointer rounded-l-lg p-2.5 text-slate-500 transition-colors hover:bg-slate-100 disabled:opacity-40"
                      disabled={cantidad <= 1}
                      aria-label="Disminuir"
                    >
                      <IconMinus className="h-4 w-4" />
                    </button>
                    <span className="w-12 text-center font-mono text-sm font-medium">{cantidad}</span>
                    <button
                      onClick={() => setCantidad((c) => Math.min(max, c + 1))}
                      className="cursor-pointer rounded-r-lg p-2.5 text-slate-500 transition-colors hover:bg-slate-100 disabled:opacity-40"
                  ...
                  <Button variant="cta" size="lg" className="flex-1" loading={adding} leftIcon={<IconCart className="h-5 w-5" />} onClick={addToCart}>
                    Agregar al carrito
                  </Button>
                </div>
              )}
              ...
              {agotado && (
                <div className="mt-6">
                  <EmptyState title="Producto agotado" message="Vuelve más tarde o explora otros productos." icon={<IconBox />} action={<Link to="/tienda"><Button variant="secondary">Ver catálogo</Button></Link>} />
                </div>
              )}
            </div>
          </div>
